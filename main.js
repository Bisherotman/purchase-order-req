...
<button type="button" class="remove">🗑</button>`;
  ...
  if (!currentUser || !userProfile) {
    ...
    return;
      ...
      if (!projectName) {
  showMsg(newMsg, "يرجى إدخال اسم المشروع", "error");
  return;
}
if (!customerName) {
  showMsg(newMsg, "يرجى إدخال اسم العميل", "error");
  return;
}
...
<button type="button" class="btn-details btn-sm" title="عرض التفاصيل" data-id="${r.tracking}" style="margin-inline-end: 6px;">🔽</button>
<button type="button" class="btn-details btn-sm" title="طباعة الطلب" data-id="${r.tracking}">🖨️</button>
  ...
  `;
    ...
    function renderAdmin() { // NEW
     ...
     // ✅ تسجيل حدث الضغط على أزرار التفاصيل
...
// ✅ دالة لتحويل كود الحالة إلى نص عربي
function statusLabel(status) {
  switch (status) {
    case "created": return "جديد";
    case "ordered": return "تم الطلب من المصنع";
    case "shipped": return "تم الشحن";
    case "partial": return "وصلت جزئياً";
    case "delivered": return "وصلت بالكامل";
    default: return "غير معروف";
  }
}

// ✅ دالة لإغلاق أو طباعة المودال
(function () {
